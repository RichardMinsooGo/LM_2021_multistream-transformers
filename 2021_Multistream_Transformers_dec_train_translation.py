...
if os.path.isfile('./checkpoints/Multistream_Transformer.pt'):
    model.load_state_dict(torch.load('./checkpoints/Multistream_Transformer.pt'))
...
for epoch in range(N_EPOCHS):
    ...
    if train_loss < best_epoch_loss:
        ...
        torch.save(model.state_dict(), './checkpoints/Multistream_Transformer.pt')
...
trained_model = MultistreamTransformer(
    num_tokens = n_dec_vocab,         # number of tokens
    dim = 512,                # dimension
    depth = 4,                # depth
    causal = True,            # autoregressive or not
...
trained_model.load_state_dict(torch.load('./checkpoints/Multistream_Transformer.pt'))
